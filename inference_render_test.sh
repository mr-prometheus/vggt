#!/bin/bash
#SBATCH -J vggt_infer_test
#SBATCH --mem=40GB
#SBATCH --gres=gpu:1 -C gmem24
#SBATCH --output=runs/inference_render_test_%j.out

# ==== CONFIGURATION ====
VIDEO_DIR="/home/c3-0/datasets/BDD_Dataset/Videos/bdd100k/videos/val/"
FILENAME_LIST="gama_list/val_day_vid.list"
TEMP_FRAMES_DIR="extracted_clips_test_tmp"
OUTPUT_DIR="/home/de575594/Deepan/CV/geolocalization/vggt-long/datasets/bdd_dataset_day/vggt-output-test-rendered"
PROGRESS_CSV="runs/inference_progress_test.csv"
...
echo "===================================================="
echo "VGGT Inference + Render Pipeline (TEST/VAL)"
echo "Video source : $VIDEO_DIR"
echo "List         : $FILENAME_LIST"
...
# Load existing CSV so we don't lose entries for videos no longer in the output dir
existing = {}
if csv_path.exists():
...
# Re-scan every subdirectory in the output dir
...
# ======================================================
TEMP_LIST="/tmp/vggt_single_$$.list"
...
while IFS= read -r line || [[ -n "$line" ]]; do
    # Test list has bare video IDs (no extension, no extra tokens)
    video_id=$(echo "$line" | tr -d '[:space:]')
    [[ -z "$video_id" ]] && continue

    # For test, the list already contains stems (no extension)
    video_stem="$video_id"
    total=$((total + 1))
...
PYEOF
    ...
    # --------------------------------------------------
    # Update progress CSV for this video
    # --------------------------------------------------
    export VGGT_VIDEO_STEM="$video_stem"
    python - <<'PYEOF'
import os, csv
