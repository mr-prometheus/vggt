...
def load_filename_list(txt_file_path: str) -> List[str]:
    """Load video IDs from file (one ID per line, no extension)"""
    filenames = []
    with open(txt_file_path, 'r') as f:
        for line in f:
            # Strip whitespace and take the entire line as the video ID
            video_id = line.strip()
            if video_id:
...
def process_videos(
    ...
    print("=" * 60)
    print("VIDEO PREPROCESSING PIPELINE - CLIP EXTRACTION (TEST)")
    print("=" * 60)
    ...
    target_video_ids = load_filename_list(filename_list_path)
    print(f"         Loaded {len(target_video_ids)} video IDs")

    # Create a set of target IDs for matching
    ...
    # Match video files by their stem (filename without extension)
    video_files = []
    for file_path in all_video_files:
        if file_path.stem in target_set:
    ...
    if len(video_files) == 0:
        ...
        print("First 5 video IDs in list:", target_video_ids[:5])
        print("First 5 video stems in directory:", [f.stem for f in all_video_files[:5]])
        return
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Extract video frames into clips (TEST SPLIT)")
    ...
    directory = "/home/c3-0/datasets/BDD_Dataset/Videos/bdd100k/videos/val/"
    filename_list = "gama_list/val_day_vid.list"

    output_csv = "metadata_clips_test.csv"
    output_frames_dir = "extracted_clips_test"
