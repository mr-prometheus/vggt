#!/bin/bash
#SBATCH -J vggt_infer_train
#SBATCH --mem=40GB
#SBATCH --gres=gpu:1 -C gmem24
#SBATCH --output=runs/inference_render_train_%j.out
...
echo "===================================================="
echo "VGGT Inference + Render Pipeline (TRAIN)"
...
while IFS= read -r line || [[ -n "$line" ]]; do
    # Take first token (handles lines like "video.mp4 label")
    video_id=$(echo "$line" | awk '{print $1}')
    [[ -z "$video_id" ]] && continue

    # Stem without extension — matches the subdir created by extract/inference
    video_stem="${video_id%.*}"
    total=$((total + 1))
...
process_videos(
    ...
    output_csv='/tmp/vggt_meta_train.csv',
